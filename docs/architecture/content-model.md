...
```typescript
const ExperienceListDataSchema = z.object({
  items: z.array(z.object({
    ...
    "items": [
      {
        ...
        "highlights": [
          "Dean's List",
          "Senior project: ML-powered code review"
        ]
      }
    ]
  }
...
```typescript
const SiteConfigDataSchema = z.object({
  name: z.string().min(1).max(100),
  title: z.string().min(1).max(200),
...
| `list` (experience) | `experience` | items[].company, items[].role, items[].startDate | items[].description, items[].endDate, items[].location, items[].type, items[].skills |
| `list` (education) | `education` | items[].institution, items[].degree, items[].startDate | items[].field, items[].endDate, items[].location, items[].gpa, items[].highlights |
